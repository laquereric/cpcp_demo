/* CPCP pull example: read access. C99 + POSIX sockets, no dependencies.
 *
 * C ships no HTTP client and no JSON parser. This file speaks just enough
 * HTTP/1.1 to POST one envelope (Content-Length framing, status line) and
 * scans the body for the "ok" marker; the full body always prints. For
 * production parsing add a JSON library -- the transport discipline below
 * does not change.
 *
 * Usage:
 *   cc -O2 -o pull pull.c && CPCP_URL=http://localhost:13002/_cpcp ./pull [method]
 *
...
static int split_url(const char *url, char *host, size_t hn, char *port, size_t pn, char *path, size_t qn) {
    ...
    /* CPCP_URL names the seam root; /rpc is appended unless already there. */
...
int main(int argc, char **argv) {
    ...
    const char *method = argc > 1 ? argv[1] : "note.list";
    ...
    snprintf(payload, sizeof payload,
             "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"%s\",\"params\":{}}", method);
    char req[4096];
